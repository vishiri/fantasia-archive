...
import { FA_APP_NOTEBOARD_STORE_DEFAULTS } from '../faAppNoteboardStoreDefaults'
...
test('Test that getFaAppNoteboard creates the store once with expected name and defaults', async () => {
  const { getFaAppNoteboard } = await import('../faAppNoteboardStore')
...
test('Test that cleanupFaAppNoteboard handles null store snapshot', async () => {
  const { cleanupFaAppNoteboard } = await import('../faAppNoteboardStore')
