...
- **Layout width/height**: In Playwright, assert rendered size with **`locator.boundingBox()`** and rounded comparisons — not via mirrored **`data-test-*`** width/height attributes or style strings alone. See the **Layout size (width and height)** section in [playwright-tests.mdc](../../rules/playwright-tests.mdc).
