...
- **Command**: `yarn test:unit` runs [vitest.config.mts](../../vitest.config.mts) (**`unit-core`**), [vitest.helpers.config.mts](../../vitest.helpers.config.mts) (**`unit-helpers`**, **`helpers/**/*.vitest.test.ts`**), then [vitest.components.config.mts](../../vitest.components.config.mts) (**`unit-components`**, Vue SFC under `src/components/**`).
...
- **Machine-readable reports**: `test-results/vitest-report/test-results-vitest.json` (core), `test-results-vitest-helpers.json` (helpers), and `test-results-vitest-components.json` (components).
...
- **Component baseline**: under `src/components/**`, maintain one colocated `tests/<ComponentName>.vitest.test.ts` per `.vue` component (add/rename/remove both together). **Extracted `scripts/*.ts`** next to an SFC should gain **`scripts/tests/*.vitest.test.ts`** when they hold real logic (same idea as `src/scripts` helpers).
