...
- **Inline code tokens**: use **`<code class="code-token">…</code>`** for the app-wide **code chip** look (see [`src/css/globals/htmlAdjustments.scss`](../../../src/css/globals/htmlAdjustments.scss)). Prefer it over new one-off inline-code styles when the default token treatment is enough. [AGENTS.md](../../../AGENTS.md) **Global CSS: `code.code-token`**, [project-scss.mdc](../../rules/project-scss.mdc).
