...
2. **Quality gate** ([testing-terminal-isolation.mdc](../../rules/testing-terminal-isolation.mdc)): in **one** terminal, `yarn verify` (stop on first failure; run `yarn lint`, `yarn lint:types`, `yarn lint:style`, or `yarn test:unit` individually only while debugging). See [eslint-typescript.mdc](../../rules/eslint-typescript.mdc) for what each covers.
