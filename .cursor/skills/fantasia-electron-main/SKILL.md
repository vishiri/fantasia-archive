...
- Frameless window controls and read-only app version from main use synchronous `ipcMain.on` handlers registered in `registerFaWindowControlIpc.ts` and `registerFaAppDetailsIpc.ts`; preload calls `ipcRenderer.sendSync` with channels from `electron-ipc-bridge.ts`. Handlers resolve `BrowserWindow` via `BrowserWindow.fromWebContents(event.sender)` so the correct window is targeted when focus moves to native menus.
