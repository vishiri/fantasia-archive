...
- **Unified error reporting** — **`faActionManagerErrorReporting_manager.ts`** exports **`reportFaActionFailure`**, **`buildFaActionPayloadPreview`**, and **`normalizeFaActionError`** from **`functions/createFaActionManagerErrorReporting.ts`**. **`reportFaActionFailure`** emits exactly **one** **`console.error`** + **one** **`Notify.create`** per failure (defaults to **`type: 'negative'`**; **`FaProjectOpenFailedError`** may set **`notifyType: 'warning'`** for a softer toast while logging stays **`console.error`**). Toast captions for hard project-open failures show the **`.faproject`** path only; **`faActionManagerErrorReportingNormalize.ts`** holds **`normalizeFaActionError`**, **`readFaProjectOpenFailedShape`**, and **`resolveFaActionFailureNotifyCaption`**. **`Notify.create`** is wired through a lazy wrapper so Quasar resolves at call time.
...
- **`src/stores/S_FaUserSettings.ts`** — **`updateSettings`** **throws** on bridge rejection or save mismatch; the action manager surfaces the toast through **`saveAppSettings`**. Success **`Notify`** stays in the store.
...
- **Manager modules** — **`src/scripts/actionManager/`** (`faActionDefinitions.ts`, `faActionDefinitionHandlers.ts`, `faActionManagerRun.ts`, `faActionManagerSyncQueue.ts`, `faActionManagerErrorReporting_manager.ts`, `faActionManagerErrorReportingNormalize.ts`, `functions/createFaActionManagerErrorReporting.ts`, `faActionManagerHistory.ts`, `faActionManagerStoreBridge_manager.ts`).
...
- **`src/scripts/actionManager/_tests/faActionManagerErrorReporting.vitest.test.ts`** — payload preview (undefined, plain object, truncation, cycle), single-toast + single-console contract, store recording.
- **`src/scripts/actionManager/_tests/faActionManagerErrorReportingNormalize.vitest.test.ts`** — error normalization, project-open caption paths, whitespace-only attempted paths.
- **`src/scripts/actionManager/_tests/faActionManagerErrorReportingManager.vitest.test.ts`** — manager wiring smoke.
