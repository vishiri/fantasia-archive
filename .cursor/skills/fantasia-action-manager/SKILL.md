...
- **History** in **`src/scripts/actionManager/faActionManagerHistory.ts`** — session-only ring buffer (cap **`FA_ACTION_HISTORY_MAX = 500`** in **`S_FaActionManager`**) with helpers **`recordHistoryEnqueued`**, **`recordHistoryStarted`** / **`recordHistoryStartedFromEntry`**, **`recordHistoryCompleted`**, **`recordHistoryOverflowDrop`**, and **`snapshotActionHistory()`** (deep-cloned, sorted ascending by **`enqueuedAt`** for the monitor table).
...
- **DialogActionMonitor** **`src/components/dialogs/DialogActionMonitor/`** — **`q-table`** of the snapshot taken on **`@show`** (rows do **not** stream live; reopen for a fresh view). Status cell renders **`q-spinner-clock`** for **`running`**, **`mdi-check`** for **`success`**, **`mdi-close`** for **`failed`**, no icon for **`queued`**. Payload (if present) shows in a **`q-tooltip`** on the action cell. The story is registered with **`tags: ['skip-visual']`** so Storybook visual capture skips it.
- **Menu wiring** — **`Toggle developer tools`** menu under **Help / Info** has a sibling **`Action Monitor`** entry (no default keybind) that dispatches **`runFaAction('openActionMonitorDialog', undefined)`**.
...
- [fantasia-testing](../fantasia-testing/SKILL.md) — Vitest layout, **`yarn testbatch:verify`** flow, Playwright build-before rule.
