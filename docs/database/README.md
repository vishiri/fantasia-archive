...
| File | Scope |
|------|--------|
| [projectDB.md](projectDB.md) | Active **`.faproject`** SQLite: `PRAGMA user_version` (max **7**), `project_data` KV, content tables (incl. **`tags`** / **`document_tags`**), migration entry, main-process modules, **`FA_PROJECT_CONTENT_IPC`**, **Project Settings** worlds snapshot save (per-world template layout) |
