...
| File | Scope |
|------|--------|
| [projectDB.md](projectDB.md) | Active **`.faproject`** SQLite: `PRAGMA user_version` (max **6**), `project_data` KV, content tables, migration entry, main-process modules, **`FA_PROJECT_CONTENT_IPC`**, **Project Settings** worlds snapshot save (per-world template layout) |
