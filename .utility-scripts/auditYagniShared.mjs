// Shared YAGNI audit helpers for auditYagni.mjs and auditYagniFullTriage.mjs.
...
export const testResultsDir = path.join(repoRoot, 'test-results')

export const INCLUDE_EXT = new Set([
  '.ts', '.vue', '.mjs', '.mts', '.cts', '.js', '.cjs',
  '.json', '.scss', '.css', '.sass', '.yaml', '.yml', '.html'
])
export const EXCLUDE_EXT = new Set([
 ...
 * @returns {string}
 */
export function domainFor (relPath) {
 ...
 * @returns {string[]}
 */
export function listGitTrackedInScope () {
 ...
 * @returns {Hit[]}
 */
export function findReExportShims (productionTs) {
  const hits = []
 ...
 * @returns {Hit[]}
 */
export function findDuplicateBasenames (tsVueFiles) {
 ...
 * @returns {Hit[]}
 */
export function findThinVueCandidates (vueFiles) {
  const hits = []
  ...
  for (const rel of vueFiles) {
    ...
    if (childTags.length <= 2) {
      hits.push({
        ...
        severity: 'info',
        file: rel.replace(/\\/g, '/'),
        sample: `thin script (${scriptLines} lines), ~${childTags.length} template tag(s)`,
        triage: 'pending'
      })
    }
  }
 ...
 * @returns {Hit[]}
 */
export function findLikelyUnusedProductionFiles (tsFiles) {
  const hits = []
  ...
  for (const rel of candidates) {
    ...
    const others = matches.filter((m) => m !== norm)
    if (matches.length <= 1 && matches[0] === norm) {
      hits.push({
        id: 'Y1-unnecessary',
        severity: 'medium',
        file: norm,
        sample: `no importers found for basename "${base}"`,
        triage: 'pending'
      })
 ...
 * @returns {Hit[]}
 */
export function findSpeculativeMarkers () {
  const hits = []
  const grep = spawnSync(
    'git',
  ...
  for (const line of (grep.stdout || '').split('\n').filter(Boolean)) {
    ...
    const file = line.slice(0, colon).replace(/\\/g, '/')
 ...
 * @returns {Hit[]}
 */
export function runAllHeuristics (files) {
...
/**
 * @param {Hit[]} hits
 * @returns {Map<string, Hit[]>}
 */
export function indexHitsByFile (hits) {
  /** @type {Map<string, Hit[]>} */
  const map = new Map()
  for (const h of hits) {
    const key = h.file.replace(/\\/g, '/')
