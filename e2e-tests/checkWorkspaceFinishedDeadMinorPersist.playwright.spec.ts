...
const STATUS_PERSIST_E2E_FAPROJECT = 'e2e-finished-dead-minor-persist.faproject'
const STATUS_PERSIST_E2E_PROJECT_NAME = 'E2E finished dead minor persist project'
const STATUS_PERSIST_E2E_TAB_LABEL = 'E2E Status Flags Doc'
const OPENED_DOCUMENTS_PERSIST_SETTLE_MS = 750

let e2eStatusPersistDocumentId = ''
...
async function createE2eProjectOnWorkspaceRoute (
  ...
  await e2eSetNextProjectCreatePath(electronApplication, STATUS_PERSIST_E2E_FAPROJECT)
  await page.locator(`[data-test-locator="${selectorList.splashNew}"]`).click()
  await expect(page.locator(`[data-test-locator="${selectorList.nameInput}"]`)).toBeVisible()
  await page.locator(`[data-test-locator="${selectorList.nameInput}"]`).fill(STATUS_PERSIST_E2E_PROJECT_NAME)
  await page.locator(`[data-test-locator="${selectorList.createBtn}"]`).click()
  await e2eExpectFaActiveProjectStoreName(page, STATUS_PERSIST_E2E_PROJECT_NAME)
...
test.describe.serial('Opened documents E2E — status flags persist before cold restart', () => {
  ...
  test.beforeAll(async ({}, testInfo) => {
    ...
    const launched = await launchFaPlaywrightE2eAppWindow({
      afterIsolationResetBeforeLaunch (): void {
        tryUnlinkE2eFaprojectFixture(STATUS_PERSIST_E2E_FAPROJECT)
      },
  ...
  test('Finished dead minor toggles save and show tab markers', async () => {
    await createE2eProjectOnWorkspaceRoute(appWindow, electronApp)
    e2eStatusPersistDocumentId = await seedEditablePersistedDocumentTab(appWindow)
    expect(e2eStatusPersistDocumentId.length).toBeGreaterThan(0)

    await hydrateOpenedDocumentsAndRoute(appWindow, e2eStatusPersistDocumentId)
    await expect(
    ...
    const tab = appWindow.locator(
      `[data-test-locator="projectAppControlBar-tab-${e2eStatusPersistDocumentId}"]`
    )
...
test.describe.serial('Opened documents E2E — cold restart keeps status tab markers', () => {
  let electronApp: ElectronApplication
  let appWindow: Page
  let suiteTestInfo: TestInfo
  ...
  test('Cold restart restores finished and dead markers on tab chrome', async () => {
    expect(e2eStatusPersistDocumentId.length).toBeGreaterThan(0)

    await navigateFaPlaywrightE2eToSplashRoute(appWindow)
    await clickFaPlaywrightE2eSplashResumePrimarySegment(appWindow)
    await e2eExpectFaActiveProjectStoreName(appWindow, STATUS_PERSIST_E2E_PROJECT_NAME)
