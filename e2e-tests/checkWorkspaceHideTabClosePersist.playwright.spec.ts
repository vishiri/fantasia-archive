...
const HIDE_TAB_CLOSE_E2E_FAPROJECT = 'e2e-hide-tab-close-persist.faproject'
const HIDE_TAB_CLOSE_E2E_PROJECT_NAME = 'E2E hide tab close persist project'
const HIDE_TAB_CLOSE_E2E_TAB_LABEL = 'E2E Hide Tab Close Tab'
const USER_SETTINGS_PERSIST_SETTLE_MS = 750

let e2eHideTabCloseDocumentId = ''
...
async function createE2eProjectOnWorkspaceRoute (
  ...
  await e2eSetNextProjectCreatePath(electronApplication, HIDE_TAB_CLOSE_E2E_FAPROJECT)
  await page.locator(`[data-test-locator="${selectorList.splashNew}"]`).click()
  await expect(page.locator(`[data-test-locator="${selectorList.nameInput}"]`)).toBeVisible()
  await page.locator(`[data-test-locator="${selectorList.nameInput}"]`).fill(HIDE_TAB_CLOSE_E2E_PROJECT_NAME)
  await page.locator(`[data-test-locator="${selectorList.createBtn}"]`).click()
  await e2eExpectFaActiveProjectStoreName(page, HIDE_TAB_CLOSE_E2E_PROJECT_NAME)
...
test.describe.serial('Opened documents E2E — hide tab close button before cold restart', () => {
  ...
  test.beforeAll(async ({}, testInfo) => {
    ...
    const launched = await launchFaPlaywrightE2eAppWindow({
      afterIsolationResetBeforeLaunch (): void {
        tryUnlinkE2eFaprojectFixture(HIDE_TAB_CLOSE_E2E_FAPROJECT)
      },
  ...
  test('hideTabCloseButton user setting hides tab close buttons while tabs remain', async () => {
    await createE2eProjectOnWorkspaceRoute(appWindow, electronApp)
    await patchUserSettingsSilentlyViaPinia(appWindow, { hideTabCloseButton: true })
    e2eHideTabCloseDocumentId = await seedSinglePersistedOpenedDocumentTab(appWindow)
    expect(e2eHideTabCloseDocumentId.length).toBeGreaterThan(0)

    await hydrateOpenedDocumentsAndRoute(appWindow, e2eHideTabCloseDocumentId)
    await expect(
      appWindow.locator(
        `[data-test-locator="projectAppControlBar-tab-${e2eHideTabCloseDocumentId}"]`
      )
...
test.describe.serial('Opened documents E2E — cold restart keeps hide tab close button', () => {
  let electronApp: ElectronApplication
  let appWindow: Page
  let suiteTestInfo: TestInfo
  ...
  test('Cold restart keeps hideTabCloseButton and hides tab close buttons', async () => {
    expect(e2eHideTabCloseDocumentId.length).toBeGreaterThan(0)

    await navigateFaPlaywrightE2eToSplashRoute(appWindow)
    await clickFaPlaywrightE2eSplashResumePrimarySegment(appWindow)
    await e2eExpectFaActiveProjectStoreName(appWindow, HIDE_TAB_CLOSE_E2E_PROJECT_NAME)
