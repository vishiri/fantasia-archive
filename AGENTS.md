...
- **Vitest** for unit coverage: `yarn test:unit` runs **unit-core** (`src-electron`, `src/scripts`, `src/boot`, `src/stores`, `src/i18n`), **unit-helpers** (**`helpers/**/*.vitest.test.ts`** — add or update **`tests/*.vitest.test.ts`** when you change helper modules), then **unit-components** (`src/components/**` Vue SFC mounts). Use [vitest-tests.mdc](.cursor/rules/vitest-tests.mdc) for style and layout.
- For `src/components/**`, keep a **1:1 Vitest presence baseline**: each component `.vue` has a colocated `tests/<ComponentName>.vitest.test.ts` file. Add/rename/remove component + Vitest counterpart together. **Extracted `scripts/*.ts`** beside an SFC should include **`scripts/tests/*.vitest.test.ts`** when they contain non-trivial logic worth guarding (Pinia sync, tree updates, etc.).
