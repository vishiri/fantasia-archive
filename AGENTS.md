...
| DB | **`better-sqlite3`**; **`.faproject`** SQLite **`user_version` max 5** — see [projectDB.md](docs/database/projectDB.md) |
...
| App theme | **`appTheme`** enum (**`types/faUserSettingsAppThemeRegistry.ts`**) — flat/fantasy × light/dark; **`applyFaAppThemeToDocument`** → **`body.fa-appTheme--flat|fantasy`**, **`body--dark|light`**, **`data-fa-app-theme`**, Quasar **`Dark`**; store **`S_FaUserSettings`** |
