...
- Before any commit (or changelog edit for new work), follow this order ([testing-terminal-isolation.mdc](.cursor/rules/testing-terminal-isolation.mdc)):
  ...
  2. Verify Storybook coverage/updates for affected user-facing **`src/components/**`** (`tests/*.stories.ts`, mocks/placeholders as needed). Layout/page Storybook previews are canvas-only (no Docs requirement); see [storybook-stories.mdc](.cursor/rules/storybook-stories.mdc). Skip when the commit is **changelog-only** under the same exception as step 1 and Storybook was already satisfied for the prior work.
