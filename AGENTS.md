...
**Adding a new shortcut command**: extend **`T_faKeybindCommandId`**, **`FA_KEYBIND_COMMAND_DEFINITIONS`**, **`faKeybindRunCommand`** (**`faKeybindRunCommand.ts`**), locale **`dialogs.keybindSettings.commands.*`**, and tests for touched modules. Full step list and file pointers: **`.cursor/skills/fantasia-keybinds/SKILL.md`**.
