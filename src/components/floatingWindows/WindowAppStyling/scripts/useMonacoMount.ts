...
/**
 * Lightweight subset of the Monaco standalone editor surface used by 'WindowAppStyling'.
 * The full Monaco type tree is huge and only available after dynamic import; declaring this locally
 * keeps the renderer free of Monaco type imports unless the window is actually opened.
 */
 ...
 *
 * Lifecycle: 'mountInto' must be called once after the host '<div>' is rendered (typically when the
 * floating window becomes visible). 'disposeEditor' is invoked automatically on component unmount and on
 * window hide; calling it more than once is safe.
 */
  ...
  async function mountInto (host: HTMLElement, initialValue: string): Promise<void> {
    ...
    await Promise.resolve(
      ResultAsync.fromPromise(
        (async (): Promise<void> => {
          const wrapper = await import('app/src/components/floatingWindows/WindowAppStyling/scripts/cssMonaco')
