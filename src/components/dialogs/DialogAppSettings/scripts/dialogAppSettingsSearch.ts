...
export function useDialogAppSettingsSearchComputed (params: {
  ...
  const searchFilteredAppSettingsTree = computed((): T_appSettingsRenderTree => {
    if (!hasActiveSearchQuery.value) {
      return {}
    }

    return filterAppSettingsTreeForSearch(appSettingsTree.value, searchSettingsQuery.value)
  })
