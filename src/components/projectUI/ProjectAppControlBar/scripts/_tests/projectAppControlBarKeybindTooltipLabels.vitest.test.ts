...
test('Test that buildProjectAppControlBarKeybindTooltipLabels resolves edit and save command labels', () => {
  const labels = buildProjectAppControlBarKeybindTooltipLabels({
...
test('Test that buildProjectAppControlBarKeybindTooltipLabels returns null labels without a snapshot', () => {
  const labels = buildProjectAppControlBarKeybindTooltipLabels({
    computed: computed as <T>(getter: () => T) => I_computedRef<T>,
