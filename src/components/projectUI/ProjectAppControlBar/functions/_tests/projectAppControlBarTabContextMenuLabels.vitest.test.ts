...
import { buildProjectAppControlBarTabContextMenuLabels } from '../projectAppControlBarTabContextMenuLabels'
...
test('Test that buildProjectAppControlBarTabContextMenuLabels exposes translated menu labels', () => {
  const labels = buildProjectAppControlBarTabContextMenuLabels({
    ...
    translateCopyDocument: () => 'Copy document',
    translateCopyName: () => 'Copy name',
    translateCopyTextColor: () => 'Copy text color',
    translateAddNewDocumentUnderThis: () => 'Add new document under this',
  ...
  expect(labels.copyDocumentLabel.value).toBe('Copy document')
  expect(labels.copyNameLabel.value).toBe('Copy name')
  expect(labels.copyTextColorLabel.value).toBe('Copy text color')
  expect(labels.addNewDocumentUnderThisLabel.value).toBe('Add new document under this')
