...
const PROJECT_DOCUMENT_CONTROL_BAR_TAB_CONTEXT_MENU_TARGET_SELECTOR =
  '.projectDocumentControlBarTabs__tab'

export function createProjectDocumentControlBarTabContextMenuOpenWiring (deps: {
  ...
  onMounted(() => {
    tabElement = deps.tabContextMenuMountRef.value?.closest(
      PROJECT_DOCUMENT_CONTROL_BAR_TAB_CONTEXT_MENU_TARGET_SELECTOR
    ) ?? null
