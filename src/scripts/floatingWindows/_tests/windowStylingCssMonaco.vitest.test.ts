...
/**
 * windowStylingCssMonaco
 * Importing the wrapper sets MonacoEnvironment with a getWorker that picks the css worker for css/scss/less labels and the editor worker for everything else.
 */
test('Test that importing windowStylingCssMonaco installs a worker resolver that maps css labels to CssWorker and others to EditorWorker', async () => {
  const mod = await import('../windowStylingCssMonaco_manager')
...
test('Test that configureMonacoEnvironment is idempotent across repeat imports', async () => {
  await import('../windowStylingCssMonaco_manager')
