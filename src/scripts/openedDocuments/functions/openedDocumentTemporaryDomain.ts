 ...
 */
export function createApplyTemporaryOpenedDocumentParent (deps: {
  normalizeNullableStringFromDb: T_normalizeOpenedDocumentNullableStringFromDb
}): (
    tab: I_faOpenedDocumentTab,
    parentDocumentId: string | null
  ) => I_faOpenedDocumentTab {
  return function applyTemporaryOpenedDocumentParent (
    tab: I_faOpenedDocumentTab,
    parentDocumentId: string | null
  ): I_faOpenedDocumentTab {
    const normalizedParentDocumentId = deps.normalizeNullableStringFromDb(parentDocumentId)
    return {
      ...tab,
      parentDocumentId,
 ...
 */
export function createPromoteTemporaryOpenedDocumentTabAfterCreate (deps: {
  ...
  return function promoteTemporaryOpenedDocumentTabAfterCreate (
    tab: I_faOpenedDocumentTab,
    input: {
      documentId: string
      keepEditMode: boolean
    ...
    const savedDocumentTextColor = deps.normalizeNullableStringFromDb(
      input.savedDocumentTextColor
    )
    const savedDocumentBackgroundColor = deps.normalizeNullableStringFromDb(
    ...
    const savedParentDocumentId = deps.normalizeNullableStringFromDb(
      input.savedParentDocumentId
    )
    const savedTreeOrderNumber = input.savedTreeOrderNumber ?? deps.emptyTreeOrderNumber
