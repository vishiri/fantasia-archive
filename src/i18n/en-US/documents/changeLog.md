...
- Added dedicated security automation workflows for pull requests and repository hygiene: **Dependency Review** (blocks newly introduced high-severity vulnerable dependencies), **OSV Scanner** (switched to Google reusable OSV workflows with `push`/`pull_request`/`merge_group` + weekly scheduled scanning on `master`), and **Gitleaks** (secret-leak detection on push/PR).
