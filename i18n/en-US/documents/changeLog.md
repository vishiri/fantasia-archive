...
- **Program settings** search: **vue-i18n** placeholder (**en-US** / **fr**); the field keeps a fixed width when **clearable** shows the clear icon so the underline does not jump; **SCSS** for the search wrapper and sticky category title now uses the same letter case as the template **BEM** classes so those layout rules apply. A dimming overlay over the vertical **category** tabs appears only while the trimmed query is non-empty and fades in and out with **Vue** **Transition** using **Quasar**-bundled **Animate.css** **fadeIn** / **fadeOut**.
