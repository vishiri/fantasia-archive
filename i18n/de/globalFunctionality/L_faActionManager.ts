export default {
  actionFailed: 'Aktion fehlgeschlagen: {actionId}',
  actionTimedOut: 'Aktion nach 30 Sekunden abgebrochen.',
  queueOverflow: 'Zu viele ausstehende Aktionen; diese wurde verworfen.',
  labels: {
    closeApp: 'Fenster schließen',
    languageSwitch: 'Oberflächensprache wechseln',
    minimizeApp: 'Fenster minimieren',
    openAboutFantasiaArchiveDialog: 'Über Fantasia Archive öffnen',
    openActionMonitorDialog: 'Aktionsmonitor öffnen',
    openAdvancedSearchGuideDialog: 'Leitfaden zur erweiterten Suche öffnen',
    openChangelogDialog: 'Änderungsprotokoll öffnen',
    openKeybindSettingsDialog: 'Tastenkürzel öffnen',
    openLicenseDialog: 'Lizenz öffnen',
    openAppSettingsDialog: 'App-Einstellungen öffnen',
    openAppStylingWindow: 'Eigenes App-CSS öffnen',
    openProjectStylingDialog: 'Eigenes Projekt-CSS öffnen',
    openTipsTricksTriviaDialog: 'Tipps, Tricks & Kurioses öffnen',
    refreshWebContentsAfterLanguage: 'Fensterinhalt aktualisieren',
    reportAppNoteboardSaveFailure: 'Fehler beim Speichern des App-Noteboards melden',
    reportProjectNoteboardSaveFailure: 'Fehler beim Speichern des Projekt-Noteboards melden',
    reportAppStylingPersistFailure: 'Fehler beim Speichern des App-CSS-Fensterlayouts melden',
    reportProjectStylingSaveFailure: 'Fehler beim Speichern des Projekt-CSS melden',
    resizeApp: 'Fenstergröße ändern',
    saveKeybindSettings: 'Tastenkürzel speichern',
    saveAppSettings: 'App-Einstellungen speichern',
    saveAppStyling: 'Eigenes App-CSS speichern',
    saveProjectStyling: 'Eigenes Projekt-CSS speichern',
    showStartupTipsNotification: 'Starthinweis anzeigen',
    toggleDeveloperTools: 'Entwicklerwerkzeuge ein/aus',
    toggleAppNoteboardWindow: 'App-Noteboard-Fenster ein/aus',
    toggleProjectNoteboardWindow: 'Projekt-Noteboard-Fenster ein/aus'
  }
}
