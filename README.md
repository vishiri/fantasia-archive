...
**Project databases** ship as a single SQLite file per project with the **`.faproject`** extension. Creation, native save-location prompts, and schema initialization run in the **Electron** main process; the renderer uses **`window.faContentBridgeAPIs.projectManagement`** (**`createProject`**) over **`FA_PROJECT_MANAGEMENT_IPC`**. See [AGENTS.md](AGENTS.md) **Extending Electron APIs** and **`.cursor/skills/fantasia-sqlite-main/SKILL.md`**.
