...
**Project databases** ship as a single SQLite file per project with the **`.faproject`** extension. Desktop installers register that extension with the OS (**electron-builder** **`fileAssociations`** in **`quasar.config.ts`**); opening a file passes its path into the renderer so **`loadExistingProject`** runs like **Load project**. **`yarn quasar:dev:electron`** ignores stray **`.faproject`** paths on the command line unless you set **`FA_DEV_OPEN_FAPROJECT=1`**. Creation, native save-location prompts, and schema initialization run in the **Electron** main process; the renderer uses **`window.faContentBridgeAPIs.projectManagement`** (**`createProject`**) over **`FA_PROJECT_MANAGEMENT_IPC`**. While a project is open, main mirrors the absolute file path next to the **`better-sqlite3`** handle and routes noteboard, styling, and similar IPC through **`runWithFaProjectDatabaseForIpcAsync`** / **`runWithFaProjectDatabaseSync`** (**`src-electron/mainScripts/projectManagement/faProjectDatabaseEnsureConnected.ts`**) so brief handle loss can trigger one reopen-from-path attempt, one retry after a recoverable SQLite error, and optionally a short renderer path reply via preload **`faProjectFailsafe`**. A full session clear still runs on intentional navigations: the main window listens to **`did-start-navigation`** (main frame, not same-document) so same-document or subframe churn does not drop the database alone. See [AGENTS.md](AGENTS.md) **Extending Electron APIs**, **`.cursor/rules/fa-project-database-access.mdc`**, and **`.cursor/skills/fantasia-sqlite-main/SKILL.md`**.
