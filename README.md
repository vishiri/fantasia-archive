...
- **Repository root**: Keep the repo root for configuration (**`package.json`**, **Quasar** / **Vite** / **TypeScript** / **ESLint** configs, **`vitest.config.mts`**, **`playwright.config.ts`**, **`index.html`**), the **`vitest/`** Vitest workspace folder, **`README`**, lockfiles, and the existing **`scripts/`** automation folder (maintained entries only — see table below). Avoid adding new standalone functional **`.ts`** modules at the root; add new **non-Playwright** harness packages as subfolders under **`helpers/`**.

| Script | Role |
| --- | --- |
| `scripts/quasarBuildElectronSummarized.mjs` | Quiet **`yarn quasar:build:electron:summarized`** / **`testbatch:ensure:*`** build log |
| `scripts/playwrightWithArtifactTrim.mjs` | **`yarn test:components`** / **`yarn test:e2e`** wrapper and artifact trim |
| `scripts/auditQuasarComponentTokens.mjs` | **`yarn audit:quasar-component-tokens`** |
| `scripts/policyComplianceAudit.mjs` | **`yarn audit:policy`** |
| `scripts/auditQuasarVariablesUsage.mjs` | Optional dead feature-token report (not wired in **`package.json`**) |
| `scripts/syncI18nLocaleTreeFromEnUs.mjs` | Maintainer: sync non-**en-US** locale tree shape from **en-US** |
| `scripts/batchTranslateI18nLocales.mjs` | Maintainer: batch-translate locale modules |
...
| `yarn audit:quasar-component-tokens` | Fail if any **`$qPascalCase…`** reference has no definition under **`src/css/theme/quasar-components/`** (see **`scripts/auditQuasarComponentTokens.mjs`**). Run after refactors to Quasar override partials. |
