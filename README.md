...
| `components/<bucket>/<ComponentName>/` | Same bucket names as `src/components/` (`dialogs`, `floatingWindows`, `globals`, `elements`, `other`; **`foundation/`** has no locale tree — Storybook-only). **`ComponentName`** matches the **`src/components/`** folder (a leading underscore appears only on the three infrastructure helpers in [AGENTS.md](AGENTS.md) **Helper / wrapper SFC naming (`_` prefix)** if one ever needs copy). One or more `L_*.ts` locale modules per mirrored component |
