...
- **Playwright — New project / `.faproject` E2E** — With **`TEST_ENV: 'e2e'`**, **`e2eSetNextProjectCreatePath`** in **`helpers/playwrightHelpers_e2e/playwrightE2eProjectPaths.ts`** stages an absolute **`.faproject`** path for the next **`createProject`** call (native save dialog skipped in main; see **`src-electron/mainScripts/projectManagement/faProjectManagementE2ePathOverride.ts`**). On routes that still show welcome actions, **`getByRole('button', { name: 'Project' })`** can match several labels that contain **project**; use **`exact: true`** for the top-bar **Project** menu (see **`e2e-tests/checkNewProjectCreation.playwright.spec.ts`**).
