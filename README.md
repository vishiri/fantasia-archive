...
| `floatingWindows/` | Movable, resizable in-renderer windows (`Window*` components); frame teleported to **`document.body`** via **`FaFloatingWindowBodyTeleport`**; shared logic under `src/scripts/floatingWindows/` (**`useFaFloatingWindowFrame`**, layout, resize clamp). See [AGENTS.md](AGENTS.md) **In-renderer floating windows**. |
