...
| `components/<bucket>/<ComponentName>/` | Same bucket names as `src/components/` (`dialogs`, `floatingWindows`, `globals`, `elements`, `other`; **`foundation/`** has no locale tree — Storybook-only). One or more `L_*.ts` locale modules per mirrored component |
