...
- **State and routing**: Pinia + Vue Router. **`/`** mounts **SplashPage** under **MainLayout** while route **meta** **`faMainLayoutHideDrawer`** suppresses the left drawer; **`/home`** keeps **IndexPage** as a legacy dev workspace shell. Renderer store **`S_FaActiveProject`** tracks whether a database project session is loaded (name, session id, and absolute **`.faproject`** path when created in-app) and drives **disabled** vs **enabled** states on several **Project** / **Documents** / **Tools** menus when nothing is open yet. The **New project** modal is **`DialogNewProject`** (**`src/components/dialogs/DialogNewProject/`**), opened via **`openNewProjectDialog`** (**`faActionManager`**); locale keys live under **`dialogs.newProject`** (**`L_newProject.ts`** per language).
